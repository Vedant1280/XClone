...
import com.example.xclone.Adapters.SuggestedAccountAdapter
                        ...
                        .addListenerForSingleValueEvent(object:ValueEventListener {
                            override fun onDataChange(snapshot: DataSnapshot) {
                                for (datasnapshot in snapshot.children) {
                                    ...
                                    if ((user?.uid.toString() != FirebaseAuth.getInstance().uid.toString()) && !listOfFollowing.contains(
                                            user?.uid.toString()
                                        )
                                    ) {
                                        ...
                                        suggestedAccountAdapter = SuggestedAccountAdapter(
                                            listOfAccounts,
                                            requireContext(),
                                            this@SuggestedAccountFragment
                                        )
                                        rvSuggestedAccount.adapter = suggestedAccountAdapter
                                        rvSuggestedAccount.layoutManager = LinearLayoutManager(
                                            requireContext(),
        ...
        FirebaseDatabase.getInstance().getReference().child("users").child(FirebaseAuth.getInstance().uid.toString())
            .addListenerForSingleValueEvent(object:ValueEventListener{
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    Toast.makeText(requireContext(),"Account Followed",Toast.LENGTH_LONG).show()
                }
